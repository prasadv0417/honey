...
import { ShoppingCart, Check, ArrowLeft, Leaf, Heart } from 'lucide-react';
...
const ProductDetail = () => {
  ...
  if (error) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center bg-honeycomb">
      <h2 className="text-3xl font-bold text-brown-900 mb-4 font-['Playfair_Display']">
  ...
  const activeSize = product.sizes?.[selectedSizeIndex] || { price: product.price, stock: product.stock || 0 };
  const productImage = product.images?.[0]?.url || product.images?.[0] || 'https://images.unsplash.com/photo-1587049352847-4d431cde70c2?auto=format&fit=crop&q=80&w=800';
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="md:w-1/2 p-8 md:p-12 border-l border-honey-100 bg-honey-50/30">
              ...
              {product.sizes && product.sizes.length > 0 && (
                <div className="mb-8">
                  <h3 className="text-sm font-bold text-brown-900 uppercase tracking-wider mb-3">
                    Select Size
                  </h3>
                  <div className="flex gap-4 flex-wrap">
                    {product.sizes.map((s, index) => (
                      <button
                        key={index}
                        onClick={() => { setSelectedSizeIndex(index); setQuantity(1); }}
                ...
                <div>
                  ...
                  <div className="flex items-center border-2 border-honey-200 rounded-xl bg-white shadow-sm overflow-hidden">
                    <button
                      ...
                      className="px-4 py-3 text-brown-600 hover:bg-honey-100 transition-colors"
                    >-</button>
                    ...
                    <ul className="text-sm text-brown-700 space-y-2">
                      {product.healthBenefits.map((benefit, i) => (
                        <li key={i} className="flex items-start">
                          <span className="text-honey-500 mr-2 mt-1 text-xs">●</span>
